#ifndef __IBT_JZT_ZK_H_INCLUDE
#define __IBT_JZT_ZK_H_INCLUDE
...
6.time_t在金字塔的定义是32位，对于使用VS2005等高版本Visual C++，time_t是64位，直接使用将导致数据结构紊乱
  请在stdafx.h文件里加上如下这个定义即可。#define _USE_32BIT_TIME_T
7.从金字塔2.934以后为了增加调试方便，用户只要在命令行增加 "-debug" 启动参数即可。
